...
bool MyScene::create( std::istream & inputfile)
{
  ...
  struct readEntity // need a structure for holding entities
  {
      ...
      bool agent;
      //for chicken only
      float view_radius, k_sep, k_align, k_coherent, k_obst, k_follow, mass;
  };
  ...
  while (!inputfile.eof() && buf != "World") // Read through until the world section
  {
    ...
    if (buf != "World")
    {
      ...
      if(buf[0]=='k') //for chicken only
      {
        ...
        inputfile>>rent->view_radius>>rent->k_sep>>rent->k_align>>rent->k_coherent
                 >>rent->k_obst>>rent->k_follow>>rent->mass;
      }
    ...
    for (int j = 0; j < x; j++)		// across (column)
    {
      ...
      if (rent != NULL)		// it might not be an agent or object
      {
        ...
        if(c=='z') agent = new MyZombieAgent(true);
          ...
          agent = new MyChickenAgent(!chicken_found, rent->view_radius, rent->k_sep, rent->k_coherent, rent->k_align, rent->k_obst, rent->k_follow, rent->mass);
          chicken_found=true;
        }
